...
export const registerPost = async (req: Request, res: Response, next: NextFunction) => {
  const schema = Joi.object({
    companyName: Joi.string().max(200).required().messages({
...
export const profilePatch = async (req: Request, res: Response, next: NextFunction) => {
  const schema = Joi.object({
    companyName: Joi.string().max(200).optional().messages({
    ...
    companyEmployees: Joi.string().max(100).optional().messages({
      "string.max": "Vui lòng nhập tối đa 100 ký tự cho quy mô nhân sự!",
    }),
    workingTime: Joi.string().max(100).optional().messages({
