...
export const HeaderMenu = (props: { showMenu: boolean }) => {
  ...
  return (
    <>
      <nav className={"lg:block " + (showMenu ? "fixed top-0 left-0 w-[280px] h-full bg-[#000065] z-10" : "hidden")}>
        <ul className="flex gap-x-[30px] flex-wrap">
          {menuList.map((item, index) => {
            // Ẩn các menu phụ thuộc trạng thái đăng nhập cho đến khi load xong auth
            if (item.isLogin !== undefined && !isAuthLoaded) {
              return null;
            }
            ...
            return (
              <li
                key={index}
                className={"flex items-center gap-x-[8px] relative group/sub-1 flex-wrap lg:p-0 p-[10px] lg:w-auto w-full " + (item.isLogin !== undefined && item.isLogin !== isLogin ? "hidden" : "")}
              >
